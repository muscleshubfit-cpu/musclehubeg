...
- **الإجابة الذرية:** الصفحة الرئيسية تفتح بفقرة مباشرة أسفل العنوان الرئيسي (H1) تلخص المنصة كاملة — كلمات مفتاحية ثانوية (منصة التدريب الرقمي، خطط مخصصة، مدربون معتمدون) + الأرقام الحية (عدد التمارين والأغذية مباشرة من الكود) — مصممة لاقتناص الـ featured snippets وإجابات نماذج الذكاء الاصطناعي.
...
- **البيانات المنظمة (JSON-LD):** `Organization` + `WebSite` على مستوى الموقع كله · `FAQPage` في الرئيسية (أسئلة المالك الحرفية) · `Article` في صفحات المدونة · `Service` / `SoftwareApplication` / `BreadcrumbList` / `ItemList` في الصفحات المتخصصة — كلها مركزية في `src/lib/seo.ts`.
- **ملفات الزحف:** `robots.txt` يسمح لكل الزواحف العامة **وزواحف الذكاء الاصطناعي** (GPTBot · ClaudeBot · PerplexityBot · OAI-SearchBot…) ويحجب صفحات الإدارة والحسابات (`/admin`, `/dashboard`, `/api/`) · `sitemap.xml` ديناميكي (App Router) يغطي كل الصفحات العامة بمرايا عربية و`hreflang` متبادل، بما فيها صفحات المدربين المنشورة المعتمدة.
