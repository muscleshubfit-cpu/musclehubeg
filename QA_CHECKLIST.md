...
| H1 | ✅ **FIXED — VERIFIED on Production** — CLOSED | Implemented Option B (server-side locale detection via cookies + headers). Commit `78a0e36` (`fix: resolve locale html language and direction`). **Architecture:** middleware sets `x-pathname` header + `mhe:locale` cookie; root layout (async Server Component) reads them via `cookies()`/`headers()` and renders `<html lang dir>` dynamically; i18n provider retains client-side `useEffect` as fallback for in-page toggle; ar/layout retains RTL `<div>` as defensive safety net. **Precedence enforced:** pathname > cookie > default. **Production verification (2026-08-19):** All 6 routes return correct server-rendered HTML attributes — `/` → `lang="en" dir="ltr"`, `/memberships` → `lang="en" dir="ltr"`, `/ar/exercises` → `lang="ar" dir="rtl"`, `/ar/foods` → `lang="ar" dir="rtl"`, `/ar/memberships` → `lang="ar" dir="rtl"`, `/ar/blog` → `lang="ar" dir="rtl"`. Middleware headers confirmed: `content-language: ar-EG`, `mhe:locale=ar`, `x-pathname: /ar/exercises`. Precedence test passed: `/ar/exercises` with stale `mhe:locale=en` cookie still returns `lang="ar" dir="rtl"`. English routes with stale `mhe:locale=ar` cookie still return `lang="en" dir="ltr"`. Auth callback intact: `/auth/callback` redirects to `/` with `lang="en"`. No 500 errors, no redirect loops. |
...
**Result:** All 5 previously-open items (C5, C6, H5, M3, H1) are now CLOSED. No remaining production-readiness blockers.
