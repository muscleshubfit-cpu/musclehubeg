...
- Commit SHA: <SHA-placeholder — نمط post-push 105-118: يُسجَّل في كوميت التوثيق التالي>
- Push status: NOT PUSHED — الدفع رُفض من GitHub: «refusing to allow a Personal Access Token to create or update workflow … without workflow scope» — توكن الجلسة (fine-grained PAT) لا يحمل صلاحية Workflows، وتحديث ملفات .github/workflows/* ممنوع عبر git push وContents API معًا (403)؛ الحل (أ) المالك يضيف للتوكن صلاحية Workflows (Settings → Developer settings → Fine-grained tokens → هذا التوكن → Repository permissions → Workflows → Read and write — قيمة التوكن لا تتغير بالتعديل) ثم الدفع فورًا في الجلسة، أو (ب) المالك يطبّق الحزمة الجاهزة من جهازه (phase119.bundle + الباتشات في download/phase119-daily-single-slot/)؛ بعدها قانون §8: تمكين workflow اللغتين عبر API والتحقق الحي غدًا مع أول يوم على الفتحتين الجديدتين
