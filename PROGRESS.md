...
| 49 | الاستبيانات (تغذية + لياقة) + إصلاح شامل للتنقل والتحميل | ✅ مكتمل |
...
> - ✅ إعادة هيكلة نظام الإشعارات: استخراج `NotificationForm` مكون مشترك + `notification-templates.ts` قوالب موحدة — تم
  - ✅ إصلاح حاسم للاستبيان (3 مشاكل):
    ...
    - **أزرار التنقل الجديدة**: التالي/رجوع على كل صفحة + إرسال في صفحة المراجعة فقط (بدل حفظ مسودة + مراجعة)
    - **كشف تلقائي للخطوة الصحيحة**: عند العودة يبدأ من الخطوة المناسبة (needs_info → الخطوة المعنية، submitted → التالية)
