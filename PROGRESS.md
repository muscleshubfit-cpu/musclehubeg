...
> **آخر تحديث:** 2026-09-03 (Phase 106: **بوابة docs-parity الآلية** — «عايز حل ثابت انها متحصلش تانى خصوصاً ان ملفات التوثيق دايما بتسبب مشاكل»: اختيار الافضل من الدراسة = GitHub Actions gate بيتنفذ على سيرفرات GitHub مع كل push/PR — مستقلة عن جلسات الوكيل ومساحات الشغل المؤقتة لإنها جوه الريبو نفسه؛ docs_parity.py جديد بيشتق كل رقم من الملفات ويقارنه بكل ورقة في README/DEVELOPER_GUIDE/INDEX مع استثناء السطور التاريخية الصحيحة · migration_audit.py بقى exit 1 على أي انجراف جديد خارج baseline موثق + فهم RENAME + مسار نسبي للـ CI · الصيد الأول حيًا: 7 ادعاءات قديمة في README من Phase 105 اتصطادت واتصلحت (3× «80 SQL» → 81 + 4× «0001→0068» → 0069) وإثبات الاتجاهين: حقن عمود وهمي → exit 1 دقيق → إرجاع → exit 0) · سابقًا — Phase 105: **إصلاح حقيقة المرآة** — موافقة «go» على مرشح 105: types.ts اتصححت للشكل الحي المثبت في 99-run — المرآة الوهمية كانت سبب 42703 في 0066 v1 ووقف 0064 v1 وكسرت قائمة الصور ومؤشر الحضور بصمت؛ 0069 تلقائي يوفّق جداول 0063 في البيئات الجديدة (no-op نقي على الإنتاج) + سكريبت التدقيق بيفهم drop column (مطابق لخط الأساس 100%) + حقل ملاحظة الصورة اتشال لأنه كان بيكتب في عمود غير موجود — دراسة الحل الجذري ضد تكرار الانجراف معروضة على المالك قبل التنفيذ بتوجيهه) · سابقًا — Phase 104: **مزامنة الوثائق والوصف — docs-only** — «عايز اتاكد ان كل خصائص ومميزات المشروع مكتوبة فى وصف وهيكل المشروع بالظبط ، وصف الريبو مكتوب قديم محتاج يتعدل»: تدقيق أرقام من الملفات مباشرة كشفت قِدم README (82 صفحة مش 76 · 69 API مش 67 · 80 ملف SQL مش 73 والخريطة لحد 0068) — README اتزامن مع Phase 103b بالكامل (Admin Panel 2.0 + طبقة مدربي الموقع B2C + المشاكل المحلولة اتشالت والمرشحة المفتوحة اتوثقت كـ Phase 105) ووصف الريبو على GitHub اتكتب من جديد عبر الـ API من «منصة كوتشينج أحمد زكي» للوصف الكامل للمنصة + 10 topics) · سابقًا — Phase 103b: **تصحيح تصنيف أنواع العملاء** — «جميع العملاء مكتوب عملاء b2b وده خطاء»: ترiggers 0030A بيسجل كل عضو تحت الإدارة في coach_assignments و0067 عدّها علاقة B2B — 0068 بيحدد عميل B2B بس لما الإسناد لمدرب حقيقي، والإسناد للإدارة بقى «متابعة الإدارة» تحت أعضاء الموقع) · سابقًا — Phase 99-run: **فتح انسداد خط الترحيل** — أول نشر لـ 0064 فشل بـ 42703 على عمودين وهميين من المرآة (coach_presence.user_id + progress_photos.taken_on) فوقف الترحيل كله عند 0063 و0065/0067 ما اطبقوش أبدًا؛ الفحص الحي عمود-عمود حدد الأعمدة الحقيقية (coach_id / taken_at) واتصلحا وكل مراجع 0064-0067 اتأكدت حية قبل إعادة الدفع)
