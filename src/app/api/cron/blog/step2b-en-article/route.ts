...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // EN/AR SEPARATION: generateEnglishArticle now returns full EN data
    // (SEO + article + FAQ + image prompts + social posts + reading time).
    // NO Arabic content is generated by this call.
    const enResult = await generateEnglishArticle(
      {
        topic: qi.topic,
    ...
    // Save full EN data to bundle
    const updatedBundle = JSON.stringify({
      ...bundle,
      seo: enResult.seo,
      englishArticle: enResult.englishArticle,
      faq: enResult.faq,
      imagePrompts: enResult.imagePrompts,
      socialPosts: enResult.socialPosts,
      estimatedReadingTime: enResult.estimatedReadingTime,
    });
    ...
    return NextResponse.json({
      ...
      step: "2b",
      queueId: qi.id,
      enTitle: enResult.seo?.en?.seoTitle || "",
      wordCount: enResult.englishArticle.split(/\s+/).length,
      hasFaq: (enResult.faq?.length || 0) > 0,
      hasImagePrompts: !!enResult.imagePrompts?.featuredImage,
      hasSocialPosts: !!enResult.socialPosts?.facebook,
      source: enResult.source,
      researchUsed: {
