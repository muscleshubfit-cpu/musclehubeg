...
export function AIGenerateModal({
 ...
 {isAr ? "مولّد المحتوى بالذكاء الاصطناعي" : "AI Content Generator"}
 </h2>
 ...
 {isAr ? "أو اتركه فارغاً واستخدم الكلمة المفتاحية" : "Or leave empty and use focus keyword"}
 </p>
 </div>
 <div>
 <Label className="text-sm font-semibold">
 ...
 placeholder={isAr ? "مثال: تمارين الصدر" : "e.g. chest exercises"}
 className="mt-1.5"
 dir="ltr"
 />
 <p className="mt-1 text-[10px] text-muted-foreground">
 {isAr ? "أو اتركها فارغة واستخدم الموضوع" : "Or leave empty and use topic"}
 </p>
 </div>
 </div>
 ...
 <div>
 <Label className="text-sm font-semibold">
 {isAr ? "اللغة الافتراضية للمقال" : "Default Article Language"}
 </Label>
 <select
 value={language}
 ...
 {/* Link suggestions */}
 {(bundle.internalLinks.length > 0 || bundle.externalLinks.length > 0) && (
 <Card className="p-4">
 <h3 className="mb-2 flex items-center gap-2 text-sm font-bold">
 ...
 <li key={i}>
 <code className="font-mono text-[10px]" dir="ltr">/{l.slug}</code>
 <span className="ms-1.5 text-muted-foreground">— {l.anchorText}</span>
 </li>
 ))}
 </ul>
 </div>
 ...
 onClick={() =>
 copyText(
 [bundle.imagePrompts.featuredImage, bundle.imagePrompts.facebookImage, bundle.imagePrompts.openGraphImage].join("\n\n---\n\n"),
 ...
 <div className="space-y-2 text-xs">
 {[
 { label: isAr ? "الصورة المميزة" : "Featured", value: bundle.imagePrompts.featuredImage },
 { label: isAr ? "فيسبوك" : "Facebook", value: bundle.imagePrompts.facebookImage },
 { label: "Open Graph", value: bundle.imagePrompts.openGraphImage },
 ].map((p, i) => (
 <div key={i} className="rounded-lg border border-border/60 p-2">
 <div className="mb-1 flex items-center justify-between">
 <span className="font-semibold text-muted-foreground">{p.label}</span>
 <button
 ...
 <p className="text-[11px] text-muted-foreground" dir="ltr">{p.value}</p>
 </div>
 ))}
 </div>
 </Card>
 ...
 className="text-[10px] text-primary hover:underline"
 >
 <Copy className="h-3 w-3 inline" /> {isAr ? "نسخ" : "Copy"}
 </button>
 </div>
 <pre className="whitespace-pre-wrap text-[11px] text-muted-foreground max-h-32 overflow-y-auto scrollbar-thin" dir="auto">
 {p.value}
 </pre>
 </div>
 ))}
 ...
 <Button size="sm" className="gap-2" onClick={() => handleApply("ar")}>
 <FileText className="h-4 w-4" />
...
function ArticlePreview({ bundle, isAr }: { bundle: GeneratedBundle; isAr: boolean }) {
 ...
 {isAr ? "معاينة المقال" : "Article Preview"} ({bundle.estimatedReadingTime} {isAr ? "د" : "min"})
 </h3>
 <div className="flex gap-1">
 <button
 onClick={() => setTab("en")}
 className={`rounded-full px-3 py-1 text-xs font-medium ${
 tab === "en" ? "bg-primary text-primary-foreground" : "border border-border text-muted-foreground"
 }`}
 >
 English
 </button>
 ...
 onClick={() => setTab("ar")}
 className={`rounded-full px-3 py-1 text-xs font-medium ${
 tab === "ar" ? "bg-primary text-primary-foreground" : "border border-border text-muted-foreground"
 }`}
 >
 العربية
 </button>
